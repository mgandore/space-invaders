...
export default class BulletController {
	...
	constructor(canvas, maxBulletsAtATime, bullerColor, soundEnabled) {
		this.canvas = canvas;
		this.maxBulletsAtATime = maxBulletsAtATime
		this.bullerColor = bullerColor
	...
	shoot(x, y, velocity, timeTillNextBulletAllowed = 0) {
		if (this.timeTillNextBulletAllowed <= 0 && this.bullets.length < this.maxBulletsAtATime) {
			const bullet = new Bullet(this.canvas, x, y, velocity, this.bullerColor)
